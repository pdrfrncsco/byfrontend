...
// email/phone removidos de playerUpdateSchema — usar /contact/ endpoint
export const playerUpdateSchema = playerCreateSchema.extend({
  status: z
    .enum(['active', 'retired', 'banned', 'inactive'], {
...
export const playerVideoSchema = z
  .object({
    title: z
      .string()
      .min(2, 'O título deve ter pelo menos 2 caracteres.')
    ...
    video_type: z.enum(
      ['highlights', 'skills', 'interview', 'match_clip', 'training', 'other'],
...
export const playerAchievementSchema = z
  .object({
    ...
    level: z.enum(
      ['club', 'national', 'continental', 'international', 'world'],
