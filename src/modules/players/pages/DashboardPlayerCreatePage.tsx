...
export function DashboardPlayerCreatePage() {
  ...
  const updateMeMutation = useUpdatePlayerMe()
  ...
  const onSubmit = (data: PlayerCreateFormData) => {
    ...
    if (from === 'onboarding') {
      // When coming from onboarding, attempt to attach the profile to the authenticated user via PATCH /players/me.
      // Some backends may return 404 if no profile exists; in that case fallback to creating a new player and continue onboarding.
      updateMeMutation.mutate(payload, {
        onError: (err: unknown) => {
          const status = (err as any)?.response?.status
          if (status === 404) {
            // Profile not found on server — fallback to creating a new player resource
