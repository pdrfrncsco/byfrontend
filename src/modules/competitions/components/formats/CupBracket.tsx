...
export function CupBracket({ competitionId }: { competitionId: string }) {
  const { bracket = [], isLoading } = useCupBracket(competitionId)
  ...
  if (bracket.length === 0) {
    return (
      <div className="flex flex-col items-center gap-md py-xl text-on-surface-variant">
        ...
        <p className="text-sm">Árvore de eliminatórias vazia.</p>
        <p className="text-xs opacity-60">Os clubes devem ser registados para gerar a chave.</p>
      </div>
    )
  }
