...
export function MatchTimeline({ events, match, isLoading = false }: MatchTimelineProps) {
  ...
  return <div className="relative space-y-sm before:absolute before:bottom-2 before:left-[3.25rem] before:top-2 before:w-px before:bg-outline-variant/30">{[...events].sort((a, b) => a.minute - b.minute).map(event => { const meta = EVENT_META[event.event_type] ?? EVENT_META[event.type] ?? { label: event.event_type_label || 'Evento', icon: Activity, color: '#64748b' }; const Icon = meta.icon; const isHome = event.club === match.home_club || event.teamId === match.homeTeamId; return <div key={event.id} className="relative grid grid-cols-[3rem_1fr] gap-md rounded-xl border border-outline-variant/20 bg-surface-container p-md"><div className="z-10 flex h-8 w-8 items-center justify-center rounded-full bg-surface-container-high text-xs font-bold text-on-surface">{event.minute}&apos;</div><div className={`flex items-center gap-sm ${isHome ? '' : 'flex-row-reverse text-right'}`}><span className="flex h-8 w-8 items-center justify-center rounded-full" style={{ backgroundColor: `${meta.color}20` }}><Icon className="h-4 w-4" style={{ color: meta.color }} /></span><div><p className="text-sm font-semibold text-on-surface">{event.player_name || event.playerId || meta.label}</p><p className="text-xs text-on-surface-variant">{meta.label}{event.notes ? ` · ${event.notes}` : ''}</p></div></div></div> })}</div>
}
