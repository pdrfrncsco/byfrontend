...
import * as competitionApi from '@/modules/competitions/services/competition.api'
...
describe('useAddMatchEvent', () => {
  ...
  it('uses competition-level override for matches_suspended when provided', async () => {
    ...
    vi.spyOn(competitionApi, 'get').mockResolvedValue({ id: competitionId, suspension_rules: { violent_conduct: 2 } } as any)
    ...
    await act(async () => {
      await result.current.mutateAsync({ type: 'violent_conduct', player: 'player-2' } as any)
    })

    expect(createSuspensionSpy).toHaveBeenCalledTimes(1)
    const payload = createSuspensionSpy.mock.calls[0][1]
    expect(payload.player).toBe(createdEvent.player)
    expect(payload.suspension_type).toBe('double_yellow' || 'violent_conduct') // hook maps based on event_type; ensure payload present
